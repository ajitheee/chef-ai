...
export default function LoginPage() {
  ...
  useEffect(() => {
    ...
    if (p.get("error") === "auth") setError("That sign-in link didn't work — try again.");
  }, []);
  ...
  async function magicLink() {
    ...
    else setNote("Check your email — the sign-in link is on its way.");
  }
      ...
      <form onSubmit={signIn} className="w-full max-w-sm rounded-3xl border-2 border-[#3A2A1E] bg-[#FFFBF2] p-6 shadow-[0_10px_0_0_#3A2A1E]">
        ...
        {!supabase ? (
          ...
          <>
            ...
            <button type="button" onClick={magicLink} disabled={busy} className="mt-2 w-full rounded-full border-2 border-[#3A2A1E]/25 px-4 py-2.5 text-xs font-bold text-[#3A2A1E]/70 hover:bg-[#3A2A1E]/5 disabled:opacity-50">
