...
- If the message carries a PORTION WEIGHT or FINISHED YIELD line computed from the card, use those numbers exactly — they are the approved figures for this job. Otherwise, for COUNT PORTIONS ('2 tacos', '1 bowl'): the card's protein weights are the raw weights that go into the pan, as written. Per-serving cooked weight = card raw protein weight × the standard COOKING yield provided ÷ base portions — cooking yield only; the trim (AP→EP) yield is applied once, in the pull list, when converting raw need to as-purchased. State the result in portionSize in ≤ 12 words (e.g. '2 tacos (≈4.3 oz cooked pork)') and use it for the finished yield, so the same card gives the same order every time.
